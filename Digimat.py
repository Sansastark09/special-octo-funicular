...
def main():
    ...
    os.chdir(pyPath)
    mdb = Mdb(pathName=pyPath+'/Analysis1.cae')
    myModel = mdb.Model(name='Analysis1')
    myAssembly = myModel.rootAssembly
    mappedMeshFlag = 1
    currentMat = myModel.Material(name='Material1')
    currentMat.Density(table=((1200,),))
    currentMat.Elastic(type = ISOTROPIC,temperatureDependency=OFF, table=[(3.4e+009,0.3)])
    myModel.HomogeneousSolidSection(name='Material1_Section',material='Material1')
    currentMat = myModel.Material(name='Material2')
    currentMat.Density(table=((2500,),))
    currentMat.Elastic(type = ISOTROPIC,temperatureDependency=OFF, table=[(8.5e+010,0.2)])
    myModel.HomogeneousSolidSection(name='Material2_Section',material='Material2')
    matrixFilePath = []
    phaseIsCoating = {}
    phaseUse3DCohesive = {}
    sizeCube = []
    ...
    myAcis = mdb.openParasolid(pyPath+'/DefaultJobName_Analysis1_Phase1_uncut.xmt_txt',topology=SOLID)
    if(myAcis.numberOfParts>1):
        inst2=[]
        for j in range(1,myAcis.numberOfParts+1):
            myName = 'DefaultJobName_Analysis1_Phase1_uncut_'+str(j)
        ...
        matrixInst = myAssembly.InstanceFromBooleanMerge(name='DefaultJobName_Analysis1_Phase1_uncut',instances=inst2,keepIntersections=OFF,originalInstances=SUPRESS,domain=GEOMETRY)
    else:
        myName='DefaultJobName_Analysis1_Phase1_uncut'
        myModel.PartFromGeometryFile(myName,myAcis,THREE_D,DEFORMABLE_BODY,scale=0.001)
        instanceName = myName
    ...
    if(myAttributes['numCells']==0 and myAttributes['numWireEdges']>0):
        matrixSet = matrixPart.Set(name='SetDefaultJobName_Analysis1_Phase1_uncut',edges=matrixPart.edges)
    else:
        matrixSet = matrixPart.Set(name='SetDefaultJobName_Analysis1_Phase1_uncut',cells=matrixPart.cells)    
    ...
    if(len(inst)>1):
        ...
        matrixInst = inst[0]
        matrixPart = myModel.Part(name='Matrix',objectToCopy=myModel.parts['DefaultJobName_Analysis1_Phase1_uncut'])
    ...
    partNames['Phase2']=[]
    myAcisIncl = mdb.openParasolid(pyPath+'/DefaultJobName_Analysis1_Phase2.xmt_txt',topology=SOLID)
    ...
    if(len(inst)>1):
        phasePartInst = myAssembly.InstanceFromBooleanMerge(name='Phase2',instances=inst,keepIntersections=OFF,originalInstances=SUPPRESS, domain=GEOMETRY)
    else:
    ...
    myAssembly.Instance(name='Phase2',part=myModel.parts['Phase2'],autoOffset=OFF,dependent=ON)
    inst.append(myAssembly.instances['Phase2'])
    if len(inst)>0:
        matrixInstCut = myAssembly.InstanceFromBooleanCut(name='matrixCut',instanceToBeCut=matrixInst,cuttingInstances=inst)
    ...
    for i in range(len(inclSurf['Phase2'])):
        ...
        for j in range(len(matrixSurf)):
            ...
            s2 = matrixSurf[j]
            if(equalCentroid(c,c2,0.0025)==1 and equals(area,area2)==1):
    ...
    if(continueFlag==NO):
        mdb.saveAs(pathName=pyPath+'/Analysis1.cae')
        return 0;
    for j in range(len(partNames['Phase2'])):
        region = myModel.parts[partNames['Phase2'][j]].sets['Set'+partNames['Phase2'][j]]
        myModel.parts[partNames['Phase2'][j]].SectionAssignment(region=region,sectionName='Material2_Section', offset=0.0)
    region = myModel.parts['matrixCut'].sets['SetDefaultJobName_Analysis1_Phase1_uncut']
    myModel.parts['matrixCut'].SectionAssignment(region=region,sectionName='Material1_Section', offset=0.0)
    ...
    for j in range(len(partNames['Phase2'])):
        ...
        if(len(region.cells)==0 and len(region.edges)>0):
            ...
            try:
                myModel.materials['Material2'].hyperelastic
    ...
    region = myModel.parts['matrixCut'].sets['SetDefaultJobName_Analysis1_Phase1_uncut']
    try:
        myModel.materials['Material1'].hyperelastic
        myModel.parts['matrixCut'].setElementType(regions=(region.cells, ),elemTypes = [mesh.ElemType(elemCode=C3D10MH)])
    except (AttributeError):
        myModel.parts['matrixCut'].setElementType(regions=(region.cells, ),elemTypes = [mesh.ElemType(elemCode=C3D10M)])
    deviationFactor=0.07
    minSizeFactor=0.1
    remeshFlag = NO
    for j in range(len(partNames['Phase2'])):
        tmpPart = myModel.parts[partNames['Phase2'][j]]
        tmpPart.seedPart(size=1.25,deviationFactor=deviationFactor, minSizeFactor=minSizeFactor)
    tmpPart = myModel.parts['matrixCut']
    tmpPart.seedPart(size=1.25,deviationFactor=deviationFactor, minSizeFactor=minSizeFactor)
            ...
            if(len(unmeshedCells)>0):
                ...
                for cellToReseed in unmeshedCells:
                    if(cellToReseed.getSize() > 0.4*totalVol and len(unmeshedCells)>1):
                        ...
                        tmpPart.seedEdgeBySize(size=1.25/((nbMeshingAttempts)*2.0),edges=edgesToReseed)
    ...
    for j in range(rangeMax):
        ...
        while ((len(unmeshedCells)>0 or len(failedCells)>0) and nbMeshingAttempts<=maxNbAttempts+1):
            ...
            if(len(unmeshedCells)>0):
                ...
                for i in range(len(numberWarningElem)):
                    nbTotalWarningElem = nbTotalWarningElem+numberWarningElem[i]
    myModel.StaticStep(name='Step-1', previous='Initial', nlgeom=OFF, initialInc=1, minInc=0.001, maxInc=1, timePeriod=1, maxNumInc=100)
    ...
    b = sizeCube[1]
    c = sizeCube[2]
    myModel.DisplacementBC(name='BC-2',createStepName='Step-1',region=myAssembly.sets['refpoint4'], u1=a*0.03, distributionType=UNIFORM, fieldName='',localCsys=None)
    ...
    mdb.Job(name='Job_Analysis1',model='Analysis1',type=ANALYSIS, explicitPrecision=SINGLE,nodalOutputPrecision=SINGLE,description='',parallelizationMethodExplicit=DOMAIN,multiprocessingMode=DEFAULT,numDomains=1, userSubroutine='',numCpus=1,scratch='',echoPrint=OFF, modelPrint=OFF,contactPrint=OFF, historyPrint=OFF)
    xMid = sizeCube[0]/2
    yMid = sizeCube[1]/2
    zMid = sizeCube[2]/2
    ...
    mdb.jobs['Job_Analysis1'].writeInput(consistencyChecking=OFF)
    mdb.saveAs(pyPath+'/Analysis1.cae')
    print '********************'
    ...
    print 'Initial seed size: '+str(1.250000000000e+000)+'\n'
    if(nbMeshingAttempts>1):
        print 'Number of local seed refinement steps:'+str(nbMeshingAttempts)+'\n'
        print 'Smallest seed size: '+str(1.250000000000e+000/((nbMeshingAttempts)*2))+'\n'
